...
// -----------------------------------------------------------------------
// Phase 3 parallel-scaling profile.
//
// Mirrors `enumerate_doubly_even_parallel` but wraps each worker's seed
// loop with `Instant`-based timing and snapshots the in-flight stats
// counters so we can recover per-seed work. The kernel is conceptually
// identical — every algorithmic decision (frontier_depth, per-worker
// inf_quota, channel cap) matches the production entry. Diverges only in
// the channel item type (`(u32 seed_id, SeedFrontier)`) and the extra
// `(WorkerProfile, Vec<SeedProfile>)` shipped back through the result
// channel.
//
// Gated behind `parallel_profiling` (default OFF) so the production
// parallel build stays byte-identical.
// -----------------------------------------------------------------------
...
/// Profiling variant of [`enumerate_doubly_even_parallel`]. Returns the
/// usual `(output, stats_flat, per_k_stats)` plus a [`ParallelProfile`]
/// with one row per worker (active/idle ns, seed count) and one row per
/// seed (worker id, ns, nodes-visited proxy, classes-emitted).
///
